...
int main()
{
	const char *name = "OS_HW1_beomjinHan"; // Name of shared memory block to be passed to shm_open
	...
	while(j<itemCnt){
		...
		printf("       Consuming Item %d with value %d at Index %d\n", j, val, out); //made some spaces to easily distinguish producer and consumer messages		
		shm_unlink(name);
		out = (out+1)%bufSize;
		SetOut(out);
	...
	if (shm_unlink(name) == -1) {
		printf("Error removing %s at item %d at in %d and out %d with value %d\n",name, j, in, out, val);
		exit(-1);
	} //couldn't figure out how to get rid of this message at the end of the execution of the program.
